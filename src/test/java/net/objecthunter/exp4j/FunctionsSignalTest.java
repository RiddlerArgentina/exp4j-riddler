...
public class FunctionsSignalTest {
    ...
    public void testSinc() {
        ...
        Assertions.assertEquals(0, sinc.setVariable("x", -Math.PI).evaluate(), 1e-12);
        Assertions.assertEquals(0, sinc.setVariable("x", -2 * Math.PI).evaluate(), 1e-12);
        Assertions.assertEquals(0, sinc.setVariable("x", 2 * Math.PI).evaluate(), 1e-12);

        double x = Math.random();
        Assertions.assertEquals(Math.sin(x) / x, sinc.setVariable("x", x).evaluate(), 1e-12);
    }
    ...
    public void testSawtooth3() {
        ...
        for (; x < 10; x += 1) {
            double res = e1.setVariable("x", x).evaluate();
            Assertions.assertEquals(r, res, 1e-12);
        }
    }
